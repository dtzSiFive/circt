...
struct Driver {
  ...
  /// Helper to obtain argument/result number of destiation.
...
RefDriver RefDriver::get(Value v) {
  ...
  return RefDriver(getRefDefine(v), ref);
}
...
HWDriver HWDriver::get(Value v) {
  // Only return driver info for those at all likely to be useful for this
  // pass. Don't chase everything just to ignore.  Refs are must more sparse
  // and quickly chased so not worth rejecting there.
...
Value HWDriver::remat(PortMappingFn mapPortFn, ImplicitLocOpBuilder &builder) {
  ...
  // (No cast neeeded for current HWDriver's, getFieldRefFromValue and
  // assert)
