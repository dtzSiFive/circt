...
struct NLARemover {
  ...
  void remove(hw::HWDesignOp module) {
...
struct FIRRTLModuleExternalizer : public OpReduction<firrtl::FModuleOp> {
  void beforeReduction(hw::HWDesignOp op) override {
    nlaRemover.clear();
    symbols.clear();
    moduleSizes.clear();
  }
  void afterReduction(hw::HWDesignOp op) override { nlaRemover.remove(op); }
...
struct InstanceStubber : public OpReduction<firrtl::InstanceOp> {
  ...
  void afterReduction(hw::HWDesignOp op) override {
...
struct MemoryStubber : public OpReduction<firrtl::MemOp> {
  void beforeReduction(hw::HWDesignOp op) override { nlaRemover.clear(); }
  void afterReduction(hw::HWDesignOp op) override { nlaRemover.remove(op); }
...
struct ConnectForwarder : public Reduction {
  void beforeReduction(hw::HWDesignOp op) override { opsToErase.clear(); }
  void afterReduction(hw::HWDesignOp op) override {
