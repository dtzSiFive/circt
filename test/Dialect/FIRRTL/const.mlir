...
// CHECK-LABEL: module @ConstUInt(in %a: !firrtl.const.uint<2>) {
firrtl.module @ConstUInt(in %a: !firrtl.const.uint<2>) {}

// CHECK-LABEL: module @ConstSInt(in %a: !firrtl.const.sint<2>) {
firrtl.module @ConstSInt(in %a: !firrtl.const.sint<2>) {}

// CHECK-LABEL: module @ConstAnalog(in %a: !firrtl.const.analog<2>) {
firrtl.module @ConstAnalog(in %a: !firrtl.const.analog<2>) {}

// CHECK-LABEL: module @ConstClock(in %a: !firrtl.const.clock) {
firrtl.module @ConstClock(in %a: !firrtl.const.clock) {}

// CHECK-LABEL: module @ConstReset(in %a: !firrtl.const.reset) {
firrtl.module @ConstReset(in %a: !firrtl.const.reset) {}

// CHECK-LABEL: module @ConstAsyncReset(in %a: !firrtl.const.asyncreset) {
firrtl.module @ConstAsyncReset(in %a: !firrtl.const.asyncreset) {}

// CHECK-LABEL: module @ConstEnum(in %a: !firrtl.const.enum<a: uint<1>, b: uint<2>>) {
firrtl.module @ConstEnum(in %a: !firrtl.const.enum<a: uint<1>, b: uint<2>>) {}

// CHECK-LABEL: module @ConstVec(in %a: !firrtl.const.vector<uint<1>, 3>) {
firrtl.module @ConstVec(in %a: !firrtl.const.vector<uint<1>, 3>) {}

// CHECK-LABEL: module @ConstVecExplicitElements(in %a: !firrtl.const.vector<const.uint<1>, 3>) {
firrtl.module @ConstVecExplicitElements(in %a: !firrtl.const.vector<const.uint<1>, 3>) {}

// CHECK-LABEL: module @ConstBundle(in %a: !firrtl.const.bundle<a: uint<1>, b: sint<2>>) {
firrtl.module @ConstBundle(in %a: !firrtl.const.bundle<a: uint<1>, b: sint<2>>) {}

// CHECK-LABEL: module @MixedConstBundle(in %a: !firrtl.bundle<a: uint<1>, b: const.sint<2>>) {
firrtl.module @MixedConstBundle(in %a: !firrtl.bundle<a: uint<1>, b: const.sint<2>>) {}

// CHECK-LABEL: module @ConstBundleExplicitElements(in %a: !firrtl.const.bundle<a: const.uint<1>, b: const.sint<2>>) {
...
// CHECK-LABEL: module @ConstSubfield
firrtl.module @ConstSubfield(in %a: !firrtl.const.bundle<a: uint<1>, b: sint<2>>, out %b: !firrtl.const.uint<1>) {
  // CHECK-NEXT: [[VAL:%.+]] = subfield %a[a] : !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  // CHECK-NEXT: connect %b, [[VAL]] : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  %0 = subfield %a[a] : !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  connect %b, %0 : !firrtl.const.uint<1>, !firrtl.const.uint<1>
}

// Subfield of a mixed const bundle should always the same constness as the field type
// CHECK-LABEL: module @MixedConstSubfield
firrtl.module @MixedConstSubfield(in %a: !firrtl.bundle<a: uint<1>, b: const.sint<2>>, out %b: !firrtl.uint<1>, out %c: !firrtl.const.sint<2>) {
  // CHECK-NEXT: [[VAL0:%.+]] = subfield %a[a] : !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  // CHECK-NEXT: [[VAL1:%.+]] = subfield %a[b] : !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  // CHECK-NEXT: connect %b, [[VAL0]] : !firrtl.uint<1>, !firrtl.uint<1>
  // CHECK-NEXT: connect %c, [[VAL1]] : !firrtl.const.sint<2>, !firrtl.const.sint<2>
  %0 = subfield %a[a] : !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  %1 = subfield %a[b] : !firrtl.bundle<a: uint<1>, b: const.sint<2>>
  connect %b, %0 : !firrtl.uint<1>, !firrtl.uint<1>
  connect %c, %1 : !firrtl.const.sint<2>, !firrtl.const.sint<2>
}

// Subindex of a const vector should always have a const result
// CHECK-LABEL: module @ConstSubindex
firrtl.module @ConstSubindex(in %a: !firrtl.const.vector<uint<1>, 3>, out %b: !firrtl.const.uint<1>) {
  // CHECK-NEXT: [[VAL:%.+]] = subindex %a[1] : !firrtl.const.vector<uint<1>, 3>
  // CHECK-NEXT: connect %b, [[VAL]] : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  %0 = subindex %a[1] : !firrtl.const.vector<uint<1>, 3>
  connect %b, %0 : !firrtl.const.uint<1>, !firrtl.const.uint<1>
}

// Subindex of a non-const vector with a const element type should always have a const result
// CHECK-LABEL: module @ConstElementSubindex
firrtl.module @ConstElementSubindex(in %a: !firrtl.vector<const.uint<1>, 3>, out %b: !firrtl.const.uint<1>) {
  // CHECK-NEXT: [[VAL:%.+]] = subindex %a[1] : !firrtl.vector<const.uint<1>, 3>
  // CHECK-NEXT: connect %b, [[VAL]] : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  %0 = subindex %a[1] : !firrtl.vector<const.uint<1>, 3>
  connect %b, %0 : !firrtl.const.uint<1>, !firrtl.const.uint<1>
}

// Subaccess of a const vector should be const only if the index is const
// CHECK-LABEL: module @ConstSubaccess
firrtl.module @ConstSubaccess(in %a: !firrtl.const.vector<uint<1>, 3>, in %constIndex: !firrtl.const.uint<4>, in %dynamicIndex: !firrtl.uint<4>, out %constOut: !firrtl.const.uint<1>, out %dynamicOut: !firrtl.uint<1>) {
  // CHECK-NEXT: [[VAL0:%.+]] = subaccess %a[%constIndex] : !firrtl.const.vector<uint<1>, 3>, !firrtl.const.uint<4>
  // CHECK-NEXT: [[VAL1:%.+]] = subaccess %a[%dynamicIndex] : !firrtl.const.vector<uint<1>, 3>, !firrtl.uint<4>
  // CHECK-NEXT: connect %constOut, [[VAL0]] : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  // CHECK-NEXT: connect %dynamicOut, [[VAL1]] : !firrtl.uint<1>, !firrtl.uint<1>
  %0 = subaccess %a[%constIndex] : !firrtl.const.vector<uint<1>, 3>, !firrtl.const.uint<4>
  %1 = subaccess %a[%dynamicIndex] : !firrtl.const.vector<uint<1>, 3>, !firrtl.uint<4>
  connect %constOut, %0 : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  connect %dynamicOut, %1 : !firrtl.uint<1>, !firrtl.uint<1>
}

// Subaccess of a non-const vector with a const element type should be const only if the index is const
// CHECK-LABEL: module @ConstElementSubaccess
firrtl.module @ConstElementSubaccess(in %a: !firrtl.vector<const.uint<1>, 3>, in %constIndex: !firrtl.const.uint<4>, in %dynamicIndex: !firrtl.uint<4>, out %constOut: !firrtl.const.uint<1>, out %dynamicOut: !firrtl.uint<1>) {
  // CHECK-NEXT: [[VAL0:%.+]] = subaccess %a[%constIndex] : !firrtl.vector<const.uint<1>, 3>, !firrtl.const.uint<4>
  // CHECK-NEXT: [[VAL1:%.+]] = subaccess %a[%dynamicIndex] : !firrtl.vector<const.uint<1>, 3>, !firrtl.uint<4>
  // CHECK-NEXT: connect %constOut, [[VAL0]] : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  // CHECK-NEXT: connect %dynamicOut, [[VAL1]] : !firrtl.uint<1>, !firrtl.uint<1>
  %0 = subaccess %a[%constIndex] : !firrtl.vector<const.uint<1>, 3>, !firrtl.const.uint<4>
  %1 = subaccess %a[%dynamicIndex] : !firrtl.vector<const.uint<1>, 3>, !firrtl.uint<4>
  connect %constOut, %0 : !firrtl.const.uint<1>, !firrtl.const.uint<1>
  connect %dynamicOut, %1 : !firrtl.uint<1>, !firrtl.uint<1>
}
...
// CHECK-LABEL: module @ConstNestedElementSubaccess
firrtl.module @ConstNestedElementSubaccess(in %a: !firrtl.vector<bundle<a: const.uint<1>>, 3>, in %constIndex: !firrtl.const.uint<4>, in %dynamicIndex: !firrtl.uint<4>) {
  // CHECK-NEXT: [[VAL0:%.+]] = subaccess %a[%constIndex] : !firrtl.vector<bundle<a: const.uint<1>>, 3>, !firrtl.const.uint<4>
  // CHECK-NEXT: [[VAL1:%.+]] = subfield [[VAL0]][a] : !firrtl.bundle<a: const.uint<1>>
  %0 = subaccess %a[%constIndex] : !firrtl.vector<bundle<a: const.uint<1>>, 3>, !firrtl.const.uint<4>
  %1 = subfield %0[a] : !firrtl.bundle<a: const.uint<1>>

  // CHECK-NEXT: [[VAL2:%.+]] = subaccess %a[%dynamicIndex] : !firrtl.vector<bundle<a: const.uint<1>>, 3>, !firrtl.uint<4>
  // CHECK-NEXT: [[VAL3:%.+]] = subfield [[VAL2]][a] : !firrtl.bundle<a: uint<1>>
  %2 = subaccess %a[%dynamicIndex] : !firrtl.vector<bundle<a: const.uint<1>>, 3>, !firrtl.uint<4>
  %3 = subfield %2[a] : !firrtl.bundle<a: uint<1>>
}

// CHECK-LABEL: module @ConstSubtag
firrtl.module @ConstSubtag(in %in : !firrtl.const.enum<a: uint<1>, b: uint<2>>,
                           out %out : !firrtl.const.uint<2>) {
  // CHECK-NEXT: [[VAL:%.+]] = subtag %in[b] : !firrtl.const.enum<a: uint<1>, b: uint<2>>
  // CHECK-NEXT: connect %out, [[VAL]] : !firrtl.const.uint<2>, !firrtl.const.uint<2>
  %0 = subtag %in[b] : !firrtl.const.enum<a: uint<1>, b: uint<2>>
  connect %out, %0 : !firrtl.const.uint<2>, !firrtl.const.uint<2>
}

// CHECK-LABEL: module @ConstRegResetValue
firrtl.module @ConstRegResetValue(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset, in %resetValue: !firrtl.const.sint<1>) {
  %0 = regreset %clock, %reset, %resetValue : !firrtl.clock, !firrtl.asyncreset, !firrtl.const.sint<1>, !firrtl.sint<1>
}
...
// CHECK-LABEL: module @ConstCastToMixedConstBundle
firrtl.module @ConstCastToMixedConstBundle(in %in: !firrtl.const.bundle<a: uint<1>>, out %out: !firrtl.bundle<a: const.uint<1>>) {
  %0 = constCast %in : (!firrtl.const.bundle<a: uint<1>>) -> !firrtl.bundle<a: const.uint<1>>
  strictconnect %out, %0 : !firrtl.bundle<a: const.uint<1>>
}

// CHECK-LABEL: module @ConstCastToMixedConstVector
firrtl.module @ConstCastToMixedConstVector(in %in: !firrtl.const.vector<uint<1>, 2>, out %out: !firrtl.vector<const.uint<1>, 2>) {
  %0 = constCast %in : (!firrtl.const.vector<uint<1>, 2>) -> !firrtl.vector<const.uint<1>, 2>
  strictconnect %out, %0 : !firrtl.vector<const.uint<1>, 2>
}
...
firrtl.module @ConstRefVectorSub(in %a: !firrtl.const.vector<uint<1>, 3>, out %_a: !firrtl.probe<const.uint<1>>) {
  %0 = ref.send %a : !firrtl.const.vector<uint<1>, 3>
  %1 = ref.sub %0[0] : !firrtl.probe<const.vector<uint<1>, 3>>
  ref.define %_a, %1 : !firrtl.probe<const.uint<1>>
}
...
firrtl.module @ConstRefBundleSub(in %a: !firrtl.const.bundle<a: uint<1>, b: sint<2>>, 
  ...
  %0 = ref.send %a : !firrtl.const.bundle<a: uint<1>, b: sint<2>>
  %1 = ref.sub %0[0] : !firrtl.probe<const.bundle<a: uint<1>, b: sint<2>>>
  ref.define %_a, %1 : !firrtl.probe<const.uint<1>>
}
...
firrtl.module @PrimOpConstOperandsConstResult(in %a: !firrtl.const.uint<4>, in %b: !firrtl.const.uint<4>) {
  %0 = and %a, %b : (!firrtl.const.uint<4>, !firrtl.const.uint<4>) -> !firrtl.const.uint<4>
}
...
firrtl.module @PrimOpMixedConstOperandsNonConstResult(in %a: !firrtl.const.uint<4>, in %b: !firrtl.uint<4>) {
  %0 = and %a, %b : (!firrtl.const.uint<4>, !firrtl.uint<4>) -> !firrtl.uint<4>
}
...
firrtl.module @ElementwiseConstOperandsConstResult(in %a: !firrtl.const.vector<uint<1>, 2>, 
  ...
  %0 = elementwise_or %a, %a : (!firrtl.const.vector<uint<1>, 2>, 
                                       !firrtl.const.vector<uint<1>, 2>) 
                                        -> !firrtl.const.vector<const.uint<1>, 2>

  %1 = elementwise_and %b, %b : (!firrtl.vector<const.uint<1>, 2>, 
                                        !firrtl.vector<const.uint<1>, 2>) 
                                         -> !firrtl.vector<const.uint<1>, 2>

  %2 = elementwise_xor %a, %b : (!firrtl.const.vector<uint<1>, 2>, 
                                        !firrtl.vector<const.uint<1>, 2>) 
                                         -> !firrtl.vector<const.uint<1>, 2>
}
...
firrtl.module @ElementwiseMixedConstOperandsNonConstResult(in %a: !firrtl.const.vector<uint<1>, 2>, 
  ...
  %0 = elementwise_or %a, %b : (!firrtl.const.vector<uint<1>, 2>, 
...
firrtl.module @MuxConstConditionConstBundlesConstResult(in %p: !firrtl.const.uint<1>, 
  ...
  %0 = mux(%p, %a, %b) : (!firrtl.const.uint<1>, 
...
firrtl.module @NonConstBundleCreateConstOperands(in %a: !firrtl.const.uint<1>) {
  %0 = bundlecreate %a : (!firrtl.const.uint<1>) -> !firrtl.bundle<a: uint<1>>
}

firrtl.module @NestedConstBundleCreateConstOperands(in %a: !firrtl.const.uint<1>) {
  %0 = bundlecreate %a : (!firrtl.const.uint<1>) -> !firrtl.bundle<a: const.uint<1>>
}

firrtl.module @NonConstVectorCreateConstOperands(in %a: !firrtl.const.uint<1>) {
  %0 = vectorcreate %a : (!firrtl.const.uint<1>) -> !firrtl.vector<uint<1>, 1>
}

firrtl.module @NestedConstVectorCreateConstOperands(in %a: !firrtl.const.uint<1>) {
  %0 = vectorcreate %a : (!firrtl.const.uint<1>) -> !firrtl.vector<const.uint<1>, 1>
}

firrtl.module @NonConstEnumCreateConstOperands(in %a: !firrtl.const.uint<1>) {
  %0 = enumcreate Some(%a) : (!firrtl.const.uint<1>) -> !firrtl.enum<None: uint<0>, Some: uint<1>>
}
