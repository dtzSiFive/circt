...
firrtl.circuit "Mem" attributes {annotations = [{class = "sifive.enterprise.firrtl.ConvertMemToRegOfVecAnnotation$"}]}{
  module public @Mem(out %d : !firrtl.probe<vector<uint<8>, 8>>, out %d2 : !firrtl.probe<vector<uint<8>, 8>>) attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %dbg, %mem_read, %mem_write, %debug = mem Undefined {
      depth = 8 : i64,
      name = "mem",
    ...
    ref.define %d, %debug : !firrtl.probe<vector<uint<8>, 8>>
    ref.define %d2, %dbg : !firrtl.probe<vector<uint<8>, 8>>
  }
    // CHECK-LABEL: circuit "Mem" {
    // CHECK:         module public @Mem(
    // CHECK:           %mem_read = wire  : !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data flip: uint<8>>
    // CHECK:           %[[v0:.+]] = subfield %mem_read[addr]
    // CHECK:           %[[v1:.+]] = subfield %mem_read[en]
    // CHECK:           %[[v2:.+]] = subfield %mem_read[clk]
    // CHECK:           %[[v3:.+]] = subfield %mem_read[data]
    // CHECK:           %mem = reg %[[v6:.+]]  : !firrtl.clock, !firrtl.vector<uint<8>, 8>
    // CHECK:           %[[v23:.+]] = subaccess %mem[%[[v4:.+]]]
    // CHECK:           %invalid_ui8 = invalidvalue : !firrtl.uint<8>
    // CHECK:           strictconnect %[[v3]], %invalid_ui8 : !firrtl.uint<8>
    // CHECK:           when %[[v1]] : !firrtl.uint<1> {
    // CHECK:             strictconnect %[[v3]], %[[v23]]
    // CHECK:           }
    // CHECK:           %mem_write = wire  : !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data: uint<8>, mask: uint<1>>
    // CHECK:           %[[v5:.+]] = subfield %mem_write[addr]
    // CHECK:           %[[v6:.+]] = subfield %mem_write[en]
    // CHECK:           %[[v7:.+]] = subfield %mem_write[clk]
    // CHECK:           %[[v8:.+]] = subfield %mem_write[data]
    // CHECK:           %[[v9:.+]] = subfield %mem_write[mask]
    // CHECK:           %[[v10:.+]] = subaccess %mem[%[[v5]]]
    // CHECK:           when %[[v6]] : !firrtl.uint<1> {
    // CHECK:             when %[[v9]] : !firrtl.uint<1> {
    // CHECK:               strictconnect %[[v10]], %[[v8]] : !firrtl.uint<8>
    // CHECK:             }
    // CHECK:           }
    // CHECK:           %11 = ref.send %mem : !firrtl.vector<uint<8>, 8>
    // CHECK:           %12 = ref.send %mem : !firrtl.vector<uint<8>, 8>
    // CHECK:           ref.define %d, %12 : !firrtl.probe<vector<uint<8>, 8>>
    // CHECK:           ref.define %d2, %11 : !firrtl.probe<vector<uint<8>, 8>>


}

firrtl.circuit "Mem_Ignore" {
  module public @Mem_Ignore() attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %mem_read, %mem_write = mem Undefined {
      depth = 8 : i64,
      name = "mem",
      ...
      writeLatency = 1 : i32
        ...
        !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data: uint<8>, mask: uint<1>>
    // CHECK:      %mem_read, %mem_write = mem Undefined
...
firrtl.circuit  "GCTModule" attributes {annotations = [
  ...
  module public @GCTModule() attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %rf_read, %rf_write = mem Undefined {
      annotations = [
      ...
      writeLatency = 1 : i32
        ...
        !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data: uint<8>, mask: uint<1>>
      // CHECK-LABEL: module public @GCTModule()
      // CHECK:         %rf = reg %2
...
firrtl.circuit "WriteMask" attributes {annotations = [
  ...
  module public @WriteMask() attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
    %mem_read, %mem_write = mem Undefined {
      depth = 8 : i64,
    ...
    // CHECK-LABEL: module public @WriteMask()
    // CHECK:         %mem = reg %2  : !firrtl.clock, !firrtl.vector<vector<uint<8>, 2>, 8>
    // CHECK:         %mem_write = wire  : !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data: vector<uint<8>, 2>, mask: vector<uint<1>, 2>>
    // CHECK:         %[[v5:.+]] = subfield %mem_write[addr]
    // CHECK:         %[[v6:.+]] = subfield %mem_write[en]
    // CHECK:         %[[v7:.+]] = subfield %mem_write[clk]
    // CHECK:         %[[v8:.+]] = subfield %mem_write[data]
    // CHECK:         %[[v9:.+]] = subfield %mem_write[mask]
    // CHECK:         %[[v10:.+]] = subaccess %mem[%5] : !firrtl.vector<vector<uint<8>, 2>, 8>, !firrtl.uint<3>
    // CHECK:         %[[v11:.+]] = subindex
    // CHECK:         %[[v12:.+]] = subindex
    // CHECK:         %[[v13:.+]] = subindex
    // CHECK:         %[[v14:.+]] = subindex
    ...
    // CHECK:         %[[v16:.+]] = subindex
    // CHECK:         when %[[v6]] : !firrtl.uint<1> {
    // CHECK:           when %[[v13]] : !firrtl.uint<1> {
    // CHECK:             strictconnect %[[v11]], %[[v12]] : !firrtl.uint<8>
    // CHECK:           }
    // CHECK:           when %[[v16]] : !firrtl.uint<1> {
    // CHECK:             strictconnect %[[v14]], %[[v15]] : !firrtl.uint<8>
    // CHECK:           }
    %mem_read1, %mem_write1 = mem Undefined {
      annotations = [
      ...
      writeLatency = 1 : i32
        ...
        !firrtl.bundle<addr: uint<3>, en: uint<1>, clk: clock, data: vector<uint<8>, 2>, mask: vector<uint<1>, 2>>
    // CHECK: %mem_read_0, %mem_write_1 = mem Undefined {depth = 8 :
  }
}
...
firrtl.circuit "MemTap" attributes {annotations = [
  ...
  module public @MemTap() attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
		%rf_MPORT, %rf_io_rdata_0_MPORT, %rf_io_rdata_1_MPORT = mem sym @rf Undefined {
      annotations = [
      ...
      writeLatency = 1 : i32
        ...
        !firrtl.bundle<addr: uint<2>, en: uint<1>, clk: clock, data flip: uint<32>>
    // CHECK-LABEL: module public @MemTap()
    // CHECK:         %rf = reg sym @rf %2
...
firrtl.circuit "NLA" attributes {annotations = [
  ...
  module private @Foo() {
    // CHECK:      %old = reg sym @old
    // CHECK-SAME:   {circt.nonlocal = @path, class = "oldNLA"}
    %old_r = mem sym @old Undefined {
      annotations = [
  ...
  module public @NLA() attributes {annotations = [
    {class = "sifive.enterprise.firrtl.MarkDUTAnnotation"}
  ]} {
