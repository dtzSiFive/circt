...
// CHECK:      module @Empty(in %clock: !firrtl.clock) {
// CHECK-NEXT: }
...
firrtl.module @UnusedMemPort(in %clock: !firrtl.clock, in %addr : !firrtl.uint<1>) {
  ...
  %0 = subindex %port1_data[1] : !firrtl.vector<uint<1>, 2>
}
// CHECK:      module @UnusedMemPort(in %clock: !firrtl.clock, in %addr: !firrtl.uint<1>) {
// CHECK-NEXT: }
...
firrtl.module @InferRead(in %cond: !firrtl.uint<1>, in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, out %out : !firrtl.uint<1>, in %vec : !firrtl.vector<uint<1>, 2>) {
  // CHECK: %ram_ramport = mem sym @s1 Undefined {depth = 256 : i64, name = "ram", portNames = ["ramport"], readLatency = 0 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<8>, en: uint<1>, clk: clock, data flip: uint<1>>
  // CHECK: [[ADDR:%.*]] = subfield %ram_ramport[addr]
  // CHECK: strictconnect [[ADDR]], %invalid_ui8
  // CHECK: [[EN:%.*]] = subfield %ram_ramport[en]
  // CHECK: strictconnect [[EN]], %c0_ui1
  // CHECK: [[CLOCK:%.*]] = subfield %ram_ramport[clk]
  // CHECK: strictconnect [[CLOCK]], %invalid_clock
  // CHECK: [[DATA:%.*]] = subfield %ram_ramport[data]
  ...
  // CHECK:   strictconnect [[ADDR]], %addr
  // CHECK:   strictconnect [[EN]], %c1_ui1
  // CHECK:   strictconnect [[CLOCK]], %clock
  // CHECK: }
  when %cond : !firrtl.uint<1> {
    chirrtl.memoryport.access %ramport_port[%addr], %clock : !chirrtl.cmemoryport, !firrtl.uint<8>, !firrtl.clock
  }

  // CHECK: %node = node [[DATA]]
  %node = node %ramport_data : !firrtl.uint<1>

  // CHECK: connect %out, [[DATA]]
  connect %out, %ramport_data : !firrtl.uint<1>, !firrtl.uint<1>

  // TODO: How do you get FileCheck to accept "[[[DATA]]]"?
  // CHECK: subaccess %vec{{\[}}[[DATA]]{{\]}} : !firrtl.vector<uint<1>, 2>, !firrtl.uint<1>
  subaccess %vec[%ramport_data] : !firrtl.vector<uint<1>, 2>, !firrtl.uint<1>
}

firrtl.module @InferWrite(in %cond: !firrtl.uint<1>, in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in : !firrtl.uint<1>) {
  // CHECK: %ram_ramport = mem Undefined {depth = 256 : i64, name = "ram", portNames = ["ramport"], readLatency = 0 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<8>, en: uint<1>, clk: clock, data: uint<1>, mask: uint<1>>
  // CHECK: [[ADDR:%.*]] = subfield %ram_ramport[addr]
  // CHECK: strictconnect [[ADDR]], %invalid_ui8
  // CHECK: [[EN:%.*]] = subfield %ram_ramport[en]
  // CHECK: strictconnect [[EN]], %c0_ui1
  ...
  // CHECK: strictconnect [[CLOCK]], %invalid_clock
  // CHECK: [[DATA:%.*]] = subfield %ram_ramport[data]
  // CHECK: strictconnect [[DATA]], %invalid_ui1
  // CHECK: [[MASK:%.*]] = subfield %ram_ramport[mask]
  // CHECK: strictconnect [[MASK]], %invalid_ui1
  ...
  // CHECK:   strictconnect [[EN]], %c1_ui1
  // CHECK:   strictconnect [[CLOCK]], %clock
  // CHECK:   strictconnect [[MASK]], %c0_ui1
  // CHECK: }
  when %cond : !firrtl.uint<1> {
    chirrtl.memoryport.access %ramport_port[%addr], %clock : !chirrtl.cmemoryport, !firrtl.uint<8>, !firrtl.clock
  }

  // CHECK: strictconnect [[MASK]], %c1_ui1
  // CHECK: connect [[DATA]], %in
  connect %ramport_data, %in : !firrtl.uint<1>, !firrtl.uint<1>
...
firrtl.module @InferReadWrite(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in : !firrtl.uint<1>, out %out : !firrtl.uint<1>) {
  // CHECK: %ram_ramport = mem Undefined {depth = 256 : i64, name = "ram", portNames = ["ramport"], readLatency = 0 : i32, writeLatency = 1 : i32} : !firrtl.bundle<addr: uint<8>, en: uint<1>, clk: clock, rdata flip: uint<1>, wmode: uint<1>, wdata: uint<1>, wmask: uint<1>>
  // CHECK: [[ADDR:%.*]] = subfield %ram_ramport[addr]
  // CHECK: strictconnect [[ADDR]], %invalid
  // CHECK: [[EN:%.*]] = subfield %ram_ramport[en]
  // CHECK: strictconnect [[EN]], %c0_ui1
  // CHECK: [[CLOCK:%.*]] = subfield %ram_ramport[clk]
  // CHECK: strictconnect [[CLOCK]], %invalid_clock
  // CHECK: [[RDATA:%.*]] = subfield %ram_ramport[rdata]
  // CHECK: [[WMODE:%.*]] = subfield %ram_ramport[wmode]
  // CHECK: strictconnect [[WMODE]], %c0_ui1
  // CHECK: [[WDATA:%.*]] = subfield %ram_ramport[wdata]
  // CHECK: strictconnect [[WDATA]], %invalid_ui1
  // CHECK: [[WMASK:%.*]] = subfield %ram_ramport[wmask]
  // CHECK: strictconnect [[WMASK]], %invalid
  %ram = chirrtl.combmem : !chirrtl.cmemory<uint<1>, 256>

  // CHECK: strictconnect [[ADDR]], %addr : !firrtl.uint<8>
  // CHECK: strictconnect [[EN]], %c1_ui1
  // CHECK: strictconnect [[CLOCK]], %clock
  // CHECK: strictconnect [[WMASK]], %c0_ui1
  ...
  // CHECK: strictconnect [[WMASK]], %c1_ui1
  // CHECK: strictconnect [[WMODE]], %c1_ui1
  // CHECK: connect [[WDATA]], %in
  connect %ramport_data, %in : !firrtl.uint<1>, !firrtl.uint<1>

  // CHECK: connect %out, [[RDATA]] 
  connect %out, %ramport_data : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @WriteToSubfield(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %value: !firrtl.uint<1>) {
  ...
  %ramport_b = subfield %ramport_data[b] : !firrtl.bundle<a: uint<1>, b: uint<1>>
  // Check that only the subfield of the mask is written to.
  // CHECK: [[DATA:%.*]] = subfield %ram_ramport[data]
  // CHECK: [[MASK:%.*]] = subfield %ram_ramport[mask]
  // CHECK: [[DATA_B:%.*]] = subfield [[DATA]][b]
  // CHECK: [[MASK_B:%.*]] = subfield [[MASK]][b]
  // CHECK: strictconnect [[MASK_B]], %c1_ui1
  // CHECK: connect [[DATA_B]], %value
  connect %ramport_b, %value : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @ReadAndWriteToSubfield(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in: !firrtl.uint<1>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[WDATA:%.*]] = subfield %ram_ramport[wdata]
  // CHECK: [[WMASK:%.*]] = subfield %ram_ramport[wmask]
  // CHECK: [[WDATA_A:%.*]] = subfield [[WDATA]][a]
  // CHECK: [[WMASK_A:%.*]] = subfield [[WMASK]][a]
  // CHECK: strictconnect [[WMASK_A]], %c1_ui1
  // CHECK: strictconnect [[WMODE]], %c1_ui1
  // CHECK: connect [[WDATA_A]], %in
  %port_a = subfield %ramport_data[a] : !firrtl.bundle<a: uint<1>, b: uint<1>>
  connect %port_a, %in : !firrtl.uint<1>, !firrtl.uint<1>

  // CHECK: [[RDATA_B:%.*]] = subfield [[RDATA]][b] : !firrtl.bundle<a: uint<1>, b: uint<1>>
  // CHECK: connect %out, [[RDATA_B]] : !firrtl.uint<1>, !firrtl.uint<1>
  %port_b = subfield %ramport_data[b] : !firrtl.bundle<a: uint<1>, b: uint<1>>
  connect %out, %port_b : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @ReadAndWriteToSubindex(in %clock: !firrtl.clock, in %addr: !firrtl.uint<8>, in %in: !firrtl.uint<1>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[WDATA:%.*]] = subfield %ram_ramport[wdata]
  // CHECK: [[WMASK:%.*]] = subfield %ram_ramport[wmask]
  // CHECK: [[WDATA_0:%.*]] = subindex [[WDATA]][0]
  // CHECK: [[WMASK_0:%.*]] = subindex [[WMASK]][0]
  // CHECK: strictconnect [[WMASK_0]], %c1_ui1 : !firrtl.uint<1>
  // CHECK: strictconnect [[WMODE]], %c1_ui1 : !firrtl.uint<1>
  // CHECK: connect [[WDATA_0]], %in : !firrtl.uint<1>, !firrtl.uint<1>
  %port_a = subindex %ramport_data[0] : !firrtl.vector<uint<1>, 10>
  connect %port_a, %in : !firrtl.uint<1>, !firrtl.uint<1>

  // CHECK: [[RDATA_I:%.*]] = subaccess [[RDATA]][%addr]
  // CHECK: connect %out, [[RDATA_I]]
  %port_b = subaccess %ramport_data[%addr] : !firrtl.vector<uint<1>, 10>, !firrtl.uint<8>
  connect %out, %port_b : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @EnableInference0(in %p: !firrtl.uint<1>, in %addr: !firrtl.uint<4>, in %clock: !firrtl.clock, out %v: !firrtl.uint<32>) {
  ...
  connect %w, %invalid_ui4 : !firrtl.uint<4>, !firrtl.uint<4>
  ...
  when %p : !firrtl.uint<1> {
    // CHECK-NEXT: strictconnect [[EN]], %c1_ui1
    // CHECK-NEXT: connect %w, %addr
    connect %w, %addr : !firrtl.uint<4>, !firrtl.uint<4>
  }
  connect %v, %ramport_data : !firrtl.uint<32>, !firrtl.uint<32>
}
...
firrtl.module @EnableInference1(in %p: !firrtl.uint<1>, in %addr: !firrtl.uint<4>, in %clock: !firrtl.clock, out %v: !firrtl.uint<32>) {
  %ram = chirrtl.seqmem Undefined  : !chirrtl.cmemory<uint<32>, 16>
  %invalid_ui32 = invalidvalue : !firrtl.uint<32>
  connect %v, %invalid_ui32 : !firrtl.uint<32>, !firrtl.uint<32>
  // CHECK: [[ADDR:%.*]] = subfield %ram_ramport[addr]
  // CHECK: [[EN:%.*]] = subfield %ram_ramport[en]
  // CHECK: when %p : !firrtl.uint<1>
  when %p : !firrtl.uint<1> {
   // CHECK-NEXT: strictconnect [[EN]], %c1_ui1
   // CHECK-NEXT: %n = node %addr
   // CHECK-NEXT: strictconnect [[ADDR]], %n
   // CHECK-NEXT: strictconnect %2, %clock
   // CHECK-NEXT: connect %v, %3
    %n = node %addr : !firrtl.uint<4>
    %ramport_data, %ramport_port = chirrtl.memoryport Read %ram {name = "ramport"} : (!chirrtl.cmemory<uint<32>, 16>) -> (!firrtl.uint<32>, !chirrtl.cmemoryport)
...
firrtl.module @EnableInference2(in %clock: !firrtl.clock, in %io: !firrtl.bundle<addr: uint<3>>, out %out: !firrtl.uint<8>) {
  ...
  connect %out, %read_data : !firrtl.uint<8>, !firrtl.uint<8>
  // CHECK: [[EN:%.*]] = subfield %mem_read[en]
  // CHECK: strictconnect [[EN]], %c0_ui1
  // CHECK: strictconnect [[EN]], %c1_ui1
}
...
firrtl.module @AddressLargerThanPort(in %clock: !firrtl.clock, in %addr: !firrtl.uint<3>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: [[ADDR:%.+]] = subfield %mem_r[addr]
  %addr_node = node %addr  : !firrtl.uint<3>
  // CHECK: [[TRUNC:%.+]] = tail %addr_node, 1
  // CHECK: strictconnect [[ADDR]], [[TRUNC]]
  chirrtl.memoryport.access %r_port[%addr_node], %clock : !chirrtl.cmemoryport, !firrtl.uint<3>, !firrtl.clock
  // CHECK: connect
  connect %out, %r_data : !firrtl.uint<1>, !firrtl.uint<1>
}
...
firrtl.module @LargeMem(in %clock: !firrtl.clock, in %addr: !firrtl.uint<35>, out %out: !firrtl.uint<1>) {
  ...
  // CHECK: %testharness_r = mem Undefined  {depth = 34359738368 : i64, name = "testharness"
  ...
  %addr_node = node %addr  : !firrtl.uint<35>
...
firrtl.module @DbgsMemPort(in %clock: !firrtl.clock, in %addr : !firrtl.uint<1>, out %_a: !firrtl.probe<vector<uint<1>, 2>>, in %cond : !firrtl.uint<1>) {
  ...
  ref.define %_a, %port0_data : !firrtl.probe<vector<uint<1>, 2>>
  // CHECK:    %[[ram_port0:.+]], %ram_ramport = mem Undefined {depth = 2 : i64, name = "ram", portNames = ["port0", "ramport"], readLatency = 0 : i32, writeLatency = 1 : i32} : !firrtl.probe<vector<uint<1>, 2>>, !firrtl.bundle<addr: uint<1>, en: uint<1>, clk: clock, data flip: uint<1>>
  // CHECK:    ref.define %_a, %[[ram_port0]] : !firrtl.probe<vector<uint<1>, 2>>
}
