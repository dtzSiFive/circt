...
firrtl.module @test(in %a : !firrtl.bundle<a: uint<1>>, out %b : !firrtl.bundle<a flip: uint<1>>) {
  ...
  // TODO: partialconnect produces type mismatch, saying not weakly equiv.
  // Does connect's verifier check less thoroughly?
  // FIRRTL spec says connect's must be type equiv, and (oriented) type equiv -> weak type equiv
  // 4.5: "Two bundle types are equivalent if they have the same number of fields, and both the bundles’
  //       i’th fields have matching names and orientations, as well as equivalent types"
