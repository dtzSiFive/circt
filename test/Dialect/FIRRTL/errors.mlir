...
firrtl.circuit "RWProbeRemote" {
  ...
  firrtl.module @RWProbeRemote() {
    // expected-error @below {{target is not in same module}}
...
firrtl.circuit "RWProbeTypes" {
  firrtl.module @RWProbeTypes() {
    ...
    // expected-error @below {{target resolves to '!firrtl.sint<1>' instead of expected '!firrtl.uint<1>'}}
...
firrtl.circuit "RWProbeInstance" {
  ...
  firrtl.module @RWProbeInstance() {
    ...
    // expected-error @below {{target does not resolve to a result}}
