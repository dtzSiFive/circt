// RUN: circt-opt %s -verify-diagnostics --pass-pipeline="builtin.module(hw.design(lower-seq-firrtl-init-to-sv, hw.module(lower-seq-firrtl-to-sv)))" | FileCheck %s --check-prefixes=CHECK,COMMON
// RUN: circt-opt %s -verify-diagnostics --pass-pipeline="builtin.module(hw.design(lower-seq-firrtl-init-to-sv, hw.module(lower-seq-firrtl-to-sv{disable-reg-randomization})))" | FileCheck %s --check-prefix COMMON --implicit-check-not RANDOMIZE_REG
// RUN: circt-opt %s -verify-diagnostics --pass-pipeline="builtin.module(hw.design(lower-seq-firrtl-init-to-sv, hw.module(lower-seq-firrtl-to-sv{emit-separate-always-blocks})))" | FileCheck %s --check-prefixes SEPARATE
